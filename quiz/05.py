    ...
    def update_vote_list(self):
        ...
        for block in self.devs.chain:
            if block['transaction']['type'] == 'open':
                ...
                for option in block['transaction']['data']['option']:
                    self.vote_list[id]['vote_count'][option] = 0
            elif block['transaction']['type'] == 'vote':
                id = block['transcation']['data']['id']
                self.vote_list[id]['totoal-vote'] +=1
                self.vote_list[id]['vote_count'][block['transcation']['data']['vote']] +=1
        self.update_vote()
        ...
        def update_(self):
            ...
            option1 = self.vote_list[self.current_vote_id]['option'][0]
            self.option1_button.setText(option1)
            self.option1_progressvar.setRange(0, self.bote_list[self.current_vote_id]['total_vote'])
            self.option1_button.setText(self.vote_list[self.current_vote_id]['vote_count'][option1])

            option2 = self.voye_list[self.current_vote_id]['option'][1]
            self.option2.setText(option2)
            self.option2_progressvar.setRange(0, self.bote_list[self.current_vote_id]['total_vote'])
            self.option2_button.setText(self.vote_list[self.current_vote_id]['vote_count'][option2])

            option3 = self.voye_list[self.current_vote_id]['option'][2]
            self.option3.setText(option2)
            self.option3_progressvar.setRange(0, self.bote_list[self.current_vote_id]['total_vote'])
            self.option3_button.setText(self.vote_list[self.current_vote_id]['vote_count'][option3])
        ...
        def vote1(self):
            block = {
                'transaction': {
                    'type': 'vote',
                    'data': {
                        'id': self.currnet_vote_id,
                        'vote': self.option1.button.text()
                    }
                }
            }
            ...
            for node in self.devs.nodes().copy():
                try:
                    node[0].sendall(json.dumps(block).endcode())
                except:
        ...
        def vote2(self):
            ...
            for node in self.devs.nodes().copy():
                try:
                    node[0].sendall(json.dumps1(block).endcode())
                except:
                    self.devs.nodes.remove(node)
                self.update_vote_list()
            ...
            block = {
                'transaction': {
                    ...
                    'data': {
                        ...
                        'options': [self.option1_line_edit.text(), self.option2_line_edit.text(), self.option3_line_edit.text()]it.text()]    super().__init__()
                    }
                }
            }
            block['hash'] = get_block_hash(block)
